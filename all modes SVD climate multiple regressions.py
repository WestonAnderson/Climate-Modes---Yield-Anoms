...
exec(open('/Users/weston/Desktop/Columbia/Research/Code/PyCode/general/helper_Functions/partial_reg_ind_dep.py').read())
...
for i in range(pcs.shape[0]):
    pcs[i,:] = (pcs[i,:]-np.mean(pcs[i,:]))/np.std(pcs[i,:])    

corners = {'ENSO':[-60,60,-179,179],'NAO':[20,80,-50,50],'AMM':[-60,30,-80,20],'IOD':[-40,30,20,160]}

#Regressions by season
for iS in [-5,-1,0,3,5,7,11]:#[-5,-3,-1,0,1,3,5,7,11,12,13,15]:
    yrOffset = iS//12
...
#    
    ...
    for iM in range(np.size(modes)):
        ...
        steps = 4        
        #sstMax = np.nanmax(sst_r[iM,...])*.75
        #smMax = np.nanmax(sm_r[iM,...])*.5
        smMax=15
        gphMax = 50
        lonSpc = 10
        latSpc = 10
        ...
        if modes[iM]=='ENSO':
            ...
            Qscale=1
            hw=4
        sstLvls = np.arange(-sstMax,sstMax+0.001,sstMax/(steps*1.5))
        smLvls = np.arange(-smMax,smMax+0.001,smMax/steps)
        gphLvls = np.arange(-gphMax,gphMax+0.001,gphStep)
        ...
        im11=plt.contourf(sstLons-180, sstLats,sst_r_temp,sstLvls,shading='flat',cmap='RdBu_r',latlon=True,extend='both')  
...
#        m11.plot(lonStpl,latStpl,'ko',markersize=.5)
        if modes[iM] != 'ENSO':
            cb11 = m11.colorbar(im11,"right",pad=6)
        else:
            cb11 = m11.colorbar(im11,"right",pad=2)
        cb11.ax.set_yticklabels(cb11.ax.get_yticklabels(), fontsize=24)


        im11=plt.contourf(smLons,smLats,sm_r[iM,...],smLvls,shading='flat',cmap='BrBG',latlon=True,extend='both')  
#        lonStpl,latStpl = m11(smLons[np.isfinite(sm_stpl[iM,...])],smLats[np.isfinite(sm_stpl[iM,...])]); 
#        m11.plot(lonStpl,latStpl,'ko',markersize=.25)
        if modes[iM] != 'ENSO':
            cb11 = m11.colorbar(im11,"left",pad=2); 
            cb11.ax.set_yticklabels(cb11.ax.get_yticklabels(), fontsize=24)

        im11=m11.quiver(gphLons[::2,::2],gphLats[::2,::2],Uwnd_all[iM,::2,::2],Vwnd_all[iM,::2,::2],
                        latlon=True, color='0.5',pivot='middle', units='inches',scale=Qscale,headwidth=hw)
        im11=m11.quiver(gphLons[::2,::2],gphLats[::2,::2],Uwnd_r[iM,::2,::2],Vwnd_r[iM,::2,::2],
                        latlon=True, pivot='middle', units='inches',scale=Qscale,headwidth=hw)
        plt.quiverkey(im11,0.5,1.05,1,'1 m/s',fontproperties={'size':24})
        plt.text(corners[modes[iM]][3]-5,corners[modes[iM]][1]+5,'GPH lvls: '+str(gphLvls[1]-gphLvls[0]),fontsize=24)

        im11=m11.contour(gphLons, gphLats,gph_all[iM,...],gphLvls,linewidths=2, shading='flat',colors='k',latlon=True)          
#        im11=m11.contour(gphLons, gphLats,gph_r[iM,...],cLvls, shading='flat',colors='k',latlon=True)    

        fig.set_size_inches(30,10);
        m11.drawparallels(np.arange(int(-95),int(95),latSpc),labels=[0,1,0,0],fontsize=24,linewidth=0.0)
        m11.drawmeridians(np.arange(int(-185),int(185),lonSpc),labels=[0,0,0,1], fontsize=24,linewidth=0.0)
        fig.savefig('/Users/weston/Desktop/Columbia/Research/Results/Climate Modes - Yield Anoms/SVD climate/multilinear regressions/'+
