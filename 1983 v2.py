...
sstAnomJFM = np.nanmean(sstAnom[[0,1,2,3,4,5],...],0)
sstAnomJAS = np.nanmean(sstAnom[[6,7,8,9,10,11],...],0)
...
ENSOsstJFM = ENSOsst[:,[0,1,2,3,4,5],...]
ENSOsstJFM = np.squeeze(np.nanmean(ENSOsstJFM[year-1981,...],0))
ENSOsstJAS = ENSOsst[:,[6,7,8,9,10,11],...]
ENSOsstJAS = np.squeeze(np.nanmean(ENSOsstJAS[year-1981,...],0))
...
fig = plt.figure();
ax1=plt.subplot(422);ax2=plt.subplot(424)
ax5=plt.subplot(426);ax6=plt.subplot(428)
...
ax3.set_yticklabels(y_labs[::2],size=32)
ax4.set_yticks(y_ticks[::2]);
ax4.set_yticklabels(y_labs[::2],size=32)
ax3.set_xticks([])
ax4.set_xticks([])
...
for shape_dict in m.cnts_info:
    ...
    if len(np.where(np.array(firstStates)==unidecode(str(shape_dict['NAME']).upper()))[0]) != 0:
        state_color = yldsFirst[np.array(firstStates)==unidecode(str(shape_dict['NAME']).upper())]['obsYld'].values.astype(float)[0]
...
for shape_dict in m.states_info:
    ...
    if len(np.where(np.array(firstStates)==str(shape_dict['HASC_1'][3:5]))[0]) != 0:
        state_color = yldsFirst[yldsFirst.state==str(shape_dict['HASC_1'][3:5])]['obsYld'].values.astype(float)[0]
...
for shape_dict in m.states_info:
    ...
    if len(np.where(np.array(firstStates)==unidecode(str(shape_dict['NAME_1'].upper())))[0]) != 0:
        state_color = yldsFirst[yldsFirst.state==unidecode(str(shape_dict['NAME_1'].upper()))]['obsYld'].values.astype(float)[0]
        colors[shape_dict['HASC_1']]=state_color
    else: continue#colors[shape_dict['HASC_1']]=0
ax = plt.gca() # get current axes instance
for nshape, seg in enumerate(m.states):
...
for shape_dict in m.states_info:
    ...
    if ((shape_dict['NAME_1']=='Chongqing')):
            colors[shape_dict['HASC_1']]=yldsFirst[yldsFirst.state==unidecode(str('SICHUAN'))]['obsYld'].values.astype(float)[0]
    elif len(np.where(np.array(firstStates)==unidecode(str(shape_dict['NAME_1'].upper())))[0]) != 0:
        state_color = yldsFirst[yldsFirst.state==unidecode(str(shape_dict['NAME_1'].upper()))]['obsYld'].values.astype(float)[0]
        colors[shape_dict['HASC_1']]=state_color
    else: continue#colors[shape_dict['HASC_1']]=0
ax = plt.gca() # get current axes instance
...
m1=m.pcolormesh(sstLons, sstLats,sstAnomJFM,zorder=1.1,shading='flat',cmap='RdBu_r',latlon=True,vmin=-2.1,vmax=2.1,alpha=0.6)  
...
for shape_dict in m.cnts_info:
    names.append(shape_dict['NAME'])
    if len(np.where(np.array(firstStates)==unidecode(str(shape_dict['NAME']).upper()))[0]) != 0:
        state_color = yldsFirst[np.array(firstStates)==unidecode(str(shape_dict['NAME']).upper())]['ensoYld'].values.astype(float)[0]
        colors[shape_dict['NAME']]=state_color
    #else: colors[shape_dict['NAME']]=0
ax = plt.gca() # get current axes instance
for nshape, seg in enumerate(m.cnts):
...
for shape_dict in m.states_info:
    ...
    #    continue#colors[shape_dict['HASC_1']]=0
    if len(np.where(np.array(firstStates)==str(shape_dict['HASC_1'][3:5]))[0]) != 0:
        state_color = yldsFirst[yldsFirst.state==str(shape_dict['HASC_1'][3:5])]['ensoYld'].values.astype(float)[0]
        colors[shape_dict['HASC_1']]=state_color
    else: continue#colors[shape_dict['HASC_1']]=0
ax = plt.gca() # get current axes instance
for nshape, seg in enumerate(m.states):
...
for shape_dict in m.states_info:
    ...
    #    continue#colors[shape_dict['HASC_1']]=0
    if len(np.where(np.array(firstStates)==unidecode(str(shape_dict['NAME_1'].upper())))[0]) != 0:
        state_color = yldsFirst[yldsFirst.state==unidecode(str(shape_dict['NAME_1'].upper()))]['ensoYld'].values.astype(float)[0]
        colors[shape_dict['HASC_1']]=state_color
    else: continue#colors[shape_dict['HASC_1']]=0
ax = plt.gca() # get current axes instance
for nshape, seg in enumerate(m.states):
...
#m.pcolor(lons,lats,cMsk_all,cmap='binary',zorder=1.5,latlon=True) 
m.pcolor(lons,lats,cMsk,cmap='tab20c_r',zorder=2,latlon=True) 
m1=m.pcolormesh(ensoLons, ensoLats,ENSOsstJFM,zorder=1.1,shading='flat',cmap='RdBu_r',latlon=True,vmin=-2.1,vmax=2.1,alpha=0.6)  
...
plt.sca(ax5)
m = Basemap(projection='cyl',llcrnrlat=-40,urcrnrlat=55,\
            llcrnrlon=-179,urcrnrlon=179,resolution='c',ax=ax5)
#m.pcolor(sstLons.astype(int),sstLats.astype(int),eof_sst,cmap='RdBu_r',latlon=True,vmin=-0.75,vmax=0.75)
...
m = Basemap(projection='cyl',llcrnrlat=-40,urcrnrlat=55,\
            llcrnrlon=-179,urcrnrlon=179,resolution='c',ax=ax6)
#m.pcolor(sstLons.astype(int),sstLats.astype(int),eof_sst,cmap='RdBu_r',latlon=True,vmin=-0.75,vmax=0.75)
...
plt.savefig('/Users/weston/Desktop/Columbia/Research/Results/Climate Modes - Yield Anoms/specific years/'+crop+
            str(year)+'_PCs'+str(PCs+1)+notes+'_v2.png')
plt.close()
