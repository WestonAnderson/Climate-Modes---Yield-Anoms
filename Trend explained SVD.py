...
clim = 'E' #E for ENSO, or 'NAO'
trnd = 'Obs' #Obs, ENSO, noENSO or fracENSO
SVDmon = '_janSVD'
NAOmons = 'DJF'
...
if trnd is 'Obs':
    ...
    clrMax = .01;
    clrMin=-.01
elif trnd is 'fracENSO':
    clrMax = 1;
    clrMin=-1
    varExp[trnd+'_trnd'].loc[np.abs(varExp.Obs_trnd)<0.0025]=0
    eof_states = varExp['index']
elif trnd is 'ENSO':
    clrMax = .0001;
    clrMin=-.0001
thisCMAP =  'BrBG_r'#
clrBr = np.arange(clrMin,clrMax+.001,.001)
...
for crop in crops:
    ...
    for shape_dict in m.cnts_info:
        ...
        (shape_dict['NAME']=='Uzbekistan')):
                colors[shape_dict['NAME']]=varExp[varExp['index']==unidecode(str(crop+'_USSR'))][trnd+'_trnd'].values.astype(float)[0]
        elif ((shape_dict['NAME']=='Belgium')|(shape_dict['NAME']=='Luxembourg')):
            if crop=='SOY':continue
            else:colors[shape_dict['NAME']]=varExp[varExp['index']==unidecode(str(crop+'_BELGIUM-LUXEMBOURG'))][trnd+'_trnd'].values.astype(float)[0]
            ...
            |(shape_dict['NAME']=='Macedonia')|(shape_dict['NAME']=='Serbia')):
                colors[shape_dict['NAME']]=varExp[varExp['index']==unidecode(str(crop+'_YUGOSLAVIA'))][trnd+'_trnd'].values.astype(float)[0]
        elif len(np.where(np.array(eof_states)==str(crop+'_'+shape_dict['NAME'].upper()))[0]) != 0:
            state_color = varExp[varExp['index']==str(crop+'_'+shape_dict['NAME'].upper())][trnd+'_trnd'].values.astype(float)[0]
    ...
    if clim is 'E':     
        ...
        for shape_dict in m.states_info:
            ...
            if (shape_dict['HASC_1']=='US.AK')|(shape_dict['HASC_1']=='US.DC')|(shape_dict['HASC_1']=='US.DE')|(shape_dict['HASC_1']=='US.HI'):
                ...
                state_color = varExp[varExp['index']==str(crop+'_'+shape_dict['NAME_1'].upper())][trnd+'_trnd'].values.astype(float)[0]
        ...
        for shape_dict in m.states_info:
            ...
            if len(np.where(np.array(eof_states)==str(crop+'_'+shape_dict['HASC_1'][3:5]))[0]) != 0:
                state_color = varExp[varExp['index']==str(crop+'_'+shape_dict['HASC_1'][3:5])][trnd+'_trnd'].values.astype(float)[0]
                colors[shape_dict['HASC_1']]=state_color
        ...
        for shape_dict in m.states_info:
            ...
            if ((shape_dict['NAME_1']=='Chongqing')):
                    colors[shape_dict['HASC_1']]=varExp[varExp['index']==unidecode(str(crop+'_SICHUAN'))][trnd+'_trnd'].values.astype(float)[0]
            elif len(np.where(np.array(eof_states)==str(crop+'_'+shape_dict['NAME_1'].upper()))[0]) != 0:
                state_color = varExp[varExp['index']==str(crop+'_'+shape_dict['NAME_1'].upper())][trnd+'_trnd'].values.astype(float)[0]
                colors[shape_dict['HASC_1']]=state_color
            else: continue#colors[shape_dict['HASC_1']]=0
        ...
        if crop is 'WHEAT':    
            ...
            for shape_dict in m.states_info:
                ...
                if ((shape_dict['NAME_1']=='Telangana')):
                    colors[shape_dict['HASC_1']]=varExp[varExp['index']==unidecode(str(crop+'_Andhra Pradesh'.upper()))][trnd+'_trnd'].values.astype(float)[0]
                elif ((shape_dict['NAME_1']=='Chhattisgarh')):
                    colors[shape_dict['HASC_1']]=varExp[varExp['index']==unidecode(str(crop+'_Madhya Pradesh'.upper()))][trnd+'_trnd'].values.astype(float)[0]
                elif ((shape_dict['NAME_1']=='Jharkhand')):
                    colors[shape_dict['HASC_1']]=varExp[varExp['index']==unidecode(str(crop+'_Bihar'.upper()))][trnd+'_trnd'].values.astype(float)[0]
                elif len(np.where(np.array(eof_states)==str(crop+'_'+shape_dict['NAME_1'].upper()))[0]) != 0:
                    state_color = varExp[varExp['index']==str(crop+'_'+shape_dict['NAME_1'].upper())][trnd+'_trnd'].values.astype(float)[0]
                    colors[shape_dict['HASC_1']]=state_color
                else: continue#colors[shape_dict['HASC_1']]=0
    ...
    plt.savefig('/Users/weston/Desktop/Columbia/Research/Results/Climate Modes - Yield Anoms/variance explained/'+trnd.lower()+clim+crop.lower()+'_'+ENSOyr+'_SVD'+str(PC+1)+'.png')
    plt.close()
